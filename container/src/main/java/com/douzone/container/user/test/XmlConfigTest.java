...
public class XmlConfigTest {
	...
	public static void main(String[] args) {
		...
		//XML Bean Configuration(Explicit Configration)
		//testBeanFactory02();
	...
	//XML Bean Configuration(Explicit Configration) 수동으로 bean 설정함
	private static void testApplicationContext02() {
		...
		//Bean Id가 자동으로 설정되지 않는다. Id를 명시해줘야 한다.
		//명시적으로 설정해줘야 한다. 
		//id로 빈 가져오기
		...
		//Type으로 빈 가져오기
		//같은 타입의 빈이 두개 이상 있으면 type으로 가져오면 Error가 난다.
		//1. id+ type
		//2. name + type
		...
		//2개 파라미터를 가지는 빈 가져오기 1
		user = ac.getBean("user3", User.class);
		System.out.println(user);

		//2개 파라미터를 가지는 빈 가져오기 2
